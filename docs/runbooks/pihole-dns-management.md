...
This runbook documents the Pi-hole DNS configuration and management for the infinity-node infrastructure. Pi-hole provides local DNS service discovery using the `local.infinity-node.com` domain, enabling services to be accessed by name instead of IP addresses.
...
- **Local Domain:** `local.infinity-node.com`
...
**Manual Entry (Web UI):**
1. Access Pi-hole admin: http://192.168.86.158/admin/
2. Navigate to: **Local DNS → DNS Records**
3. Add record: Hostname → IP address
4. Short names work automatically (e.g., `vm-100` resolves to `vm-100.local.infinity-node.com`)
...
- **Device:** Raspberry Pi
- **IP Address:** 192.168.86.158 (static reservation)
...
- `emby.local.infinity-node.com` → 192.168.86.172
- `portainer-100.local.infinity-node.com` → 192.168.86.172
- `tdarr.local.infinity-node.com` → 192.168.86.172

**VM 101 (downloads) Services:**
- `portainer-101.local.infinity-node.com` → 192.168.86.173
- `deluge.local.infinity-node.com` → 192.168.86.173
- `nzbget.local.infinity-node.com` → 192.168.86.173

**VM 102 (arr) Services:**
- `portainer-102.local.infinity-node.com` → 192.168.86.174
- `radarr.local.infinity-node.com` → 192.168.86.174
- `sonarr.local.infinity-node.com` → 192.168.86.174
- `prowlarr.local.infinity-node.com` → 192.168.86.174
- `lidarr.local.infinity-node.com` → 192.168.86.174
- `jellyseerr.local.infinity-node.com` → 192.168.86.174
- `huntarr.local.infinity-node.com` → 192.168.86.174
- `flaresolverr.local.infinity-node.com` → 192.168.86.174

**VM 103 (misc) Services:**
- `portainer-103.local.infinity-node.com` → 192.168.86.249
- `vaultwarden.local.infinity-node.com` → 192.168.86.249
- `audiobookshelf.local.infinity-node.com` → 192.168.86.249
- `paperless.local.infinity-node.com` → 192.168.86.249
- `immich.local.infinity-node.com` → 192.168.86.249
- `linkwarden.local.infinity-node.com` → 192.168.86.249
- `navidrome.local.infinity-node.com` → 192.168.86.249
- `homepage.local.infinity-node.com` → 192.168.86.249
- `mybibliotheca.local.infinity-node.com` → 192.168.86.249
- `calibre.local.infinity-node.com` → 192.168.86.249
...
```
<service-name>.local.infinity-node.com
```

### Examples

- **VM records:** `vm-100.local.infinity-node.com`, `vm-101.local.infinity-node.com`
- **Service records:** `emby.local.infinity-node.com`, `vaultwarden.local.infinity-node.com`
- **Portainer records:** `portainer-100.local.infinity-node.com` (includes VM number for uniqueness)

### Short Names

With "Expand hostnames" enabled in Pi-hole:
- Short name: `vm-100` → Resolves to: `vm-100.local.infinity-node.com`
- Short name: `emby` → Resolves to: `emby.local.infinity-node.com`
...
1. **Access Pi-hole Admin:**
   - Navigate to: http://192.168.86.158/admin/
...
3. **Add Record:**
   ...
   - Enter **IP Address** (e.g., `192.168.86.249`)
   - Click **Save**

4. **Verify:**
   ```bash
   dig +short new-service.local.infinity-node.com
...
```bash
# Test DNS resolution
dig +short service-name.local.infinity-node.com

# Test HTTP access
curl -I http://service-name.local.infinity-node.com:PORT
```
...
- `http://emby.local.infinity-node.com:8096`
- `http://vaultwarden.local.infinity-node.com:8111`

### Future Approach (Traefik Reverse Proxy)

When Traefik is deployed (task IN-046), services can be accessed without ports:
- `http://emby.local.infinity-node.com` (Traefik routes by hostname)
- `https://vaultwarden.local.infinity-node.com` (TLS termination)
...
dig @192.168.86.158 vm-100.local.infinity-node.com
...
- Verify router DHCP provides Pi-hole (192.168.86.158) as primary DNS
- Verify secondary DNS is set (1.1.1.1 for failover)

**Check Pi-hole status:**
- Access Pi-hole admin dashboard
- Check query log for DNS requests
- Verify local domain is configured (`local.infinity-node.com`)
...
- Primary: 192.168.86.158 (Pi-hole)
...
- Verify reservation points to 192.168.86.158
...
for vm in vm-100 vm-101 vm-102 vm-103; do
  echo "$vm.local.infinity-node.com -> $(dig +short ${vm}.local.infinity-node.com)"
done

# Test service records
dig +short emby.local.infinity-node.com
dig +short vaultwarden.local.infinity-node.com
dig +short audiobookshelf.local.infinity-node.com
```
...
curl -I http://audiobookshelf.local.infinity-node.com:13378
...
3. Filter by domain: `local.infinity-node.com`
