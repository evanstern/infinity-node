...
#
# Arguments:
#   vm-ip: IP address of the VM (e.g., vm-101.local.infinity-node.win)
#   stack-id: Optional Portainer stack ID. If not provided, will find Traefik stack automatically
#
# Examples:
#   ./fix-traefik-config-files.sh vm-101.local.infinity-node.win
#   ./fix-traefik-config-files.sh vm-103.local.infinity-node.win 53
#
# What it does:
...
if [ $# -lt 1 ]; then
    ...
    echo "" >&2
    echo "Examples:" >&2
    echo "  $0 vm-101.local.infinity-node.win" >&2
    echo "  $0 vm-103.local.infinity-node.win 53" >&2
    exit 1
fi
...
if [ -z "$VM_NUM" ]; then
    ...
    case "$VM_IP" in
        vm-100.local.infinity-node.win) VM_NUM="100" ;;
        vm-101.local.infinity-node.win) VM_NUM="101" ;;
        vm-102.local.infinity-node.win) VM_NUM="102" ;;
        vm-103.local.infinity-node.win) VM_NUM="103" ;;
