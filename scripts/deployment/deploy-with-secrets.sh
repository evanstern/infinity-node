...
log_info "Deploying ${SERVICE_NAME} to ${VM_HOST}"
...
# ssh evan@${VM_HOST} "cd ${STACK_PATH} && docker compose up -d"
# if [ $? -eq 0 ]; then
...
log_info "In production, would run: ssh evan@${VM_HOST} 'cd ${STACK_PATH} && docker compose up -d'"
...
# CONTAINER_STATUS=$(ssh evan@${VM_HOST} "docker ps --filter name=${SERVICE_NAME} --format '{{.Status}}'" || echo "")
