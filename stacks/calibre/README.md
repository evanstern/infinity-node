...
**Status**: ✅ Active
**VM**: 103 (misc) - `vm-103.local.infinity-node.win`
...
**5. Verify Health**
```bash
ssh evan@vm-103.local.infinity-node.win
...
1. Open browser to: `http://calibre.local.infinity-node.win` (port-free via Traefik) or `http://calibre.local.infinity-node.win:8265` (direct)
...
1. Open browser to: `http://calibre-web.local.infinity-node.win` (port-free via Traefik) or `http://calibre.local.infinity-node.win:8267` (direct)
...
**Via Calibre Server:**
1. Access Calibre GUI at http://calibre.local.infinity-node.win (port-free) or http://calibre.local.infinity-node.win:8265 (direct)
2. Click **Add books**
3. Select files to import
4. Calibre copies books to library and fetches metadata

**Via Calibre-Web:**
1. Access Calibre-Web at http://calibre-web.local.infinity-node.win (port-free) or http://calibre.local.infinity-node.win:8267 (direct)
...
**Symptoms:** VNC interface doesn't appear at http://calibre.local.infinity-node.win or port 8265
...
|---------|------|---------|-----|
| Calibre GUI | 8265 | VNC desktop interface | http://calibre.local.infinity-node.win (port-free) or http://calibre.local.infinity-node.win:8265 (direct) |
| Calibre Server | 8266 | Internal server (content) | Internal only (not exposed) |
| Calibre-Web | 8267 | Web reading interface | http://calibre-web.local.infinity-node.win (port-free) or http://calibre.local.infinity-node.win:8267 (direct) |
...
**Calibre Server:**
- Exposed only on local network (192.168.1.0/24)
