...
- [ ] **Audit git-tracked configs** `[agent:docker]`
...
- [ ] **Update bookmarks.yaml** `[agent:docker]`
  - Replace Portainer IPs with DNS names
  - Replace Proxmox IP with DNS name (if DNS record exists)
  - Replace NAS IP with `thor.local.infinity-node.com:5000` (DNS record exists)

- [ ] **Validate homepage config** `[agent:testing]`
  - Check YAML syntax
