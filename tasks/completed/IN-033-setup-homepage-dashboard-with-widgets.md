...
- [x] **Research Homepage widget support for each service type** `[agent:docker]`
  ...
  - **Completed**: Found widget support for Emby, Radarr, Sonarr, Lidarr, Prowlarr, Jellyseerr. Others use simple links.

- [x] **Verify current Homepage volume mount configuration** `[agent:docker]` `[risk:5]`
  ...
  - **Completed**: Config path is `/data/compose/19/stacks/homepage/config/homepage` on VM 103. Using local `stacks/homepage/config/homepage/` for git-based deployment.
...
- [ ] **Copy prismatic autumn wallpapers to config directory** `[agent:docker]` `[risk:3]`
  ...
  - **Status**: Directory structure created. Wallpapers not found in Downloads - can be added later.

- [x] **Verify directory structure and permissions** `[agent:docker]`
  - Ensure Homepage container can read config files
  - Create any needed subdirectories
  - Check file ownership matches PUID/PGID from compose file
  - **Completed**: Created `stacks/homepage/config/homepage/` locally. Structure ready for git commit.

- [x] **Update .env.example if needed** `[agent:docker]`
  - Document any new environment variables
  - Note that config files are gitignored
  - **Completed**: .env.example already documents CONFIG_PATH. Config files are NOT gitignored (only .env files are).

### Phase 2: Core Configuration

**Primary Agent**: `docker`

- [x] **Create settings.yaml with theme and layout** `[agent:docker]`
  ...
  - **Completed**: Created `stacks/homepage/config/homepage/settings.yaml` with dark theme, layout config, and placeholder for backgrounds.

- [x] **Create bookmarks.yaml with infrastructure links** `[agent:docker]`
  ...
  - **Completed**: Created `stacks/homepage/config/homepage/bookmarks.yaml` with all infrastructure links organized by category.

### Phase 3: Service Configuration

**Primary Agent**: `docker`

- [x] **Create services.yaml with all services organized by VM** `[agent:docker]` `[risk:2]`
  ...
  - **Completed**: Created `stacks/homepage/config/homepage/services.yaml` with all 19 services organized by VM:
    - VM 100: Emby (with widget placeholder)
    - VM 101: Deluge, NZBGet (links)
    - VM 102: Radarr, Sonarr, Lidarr, Prowlarr, Jellyseerr (with widget placeholders), FlareSolverr, Huntarr (links)
    - VM 103: Vaultwarden, Paperless-NGX, Immich, Linkwarden, Navidrome, Audiobookshelf, Homepage, Portainer, Watchtower (links)
    - Docker widget configured for VM 103 containers

### Phase 4: Widget Configuration

**Primary Agent**: `docker`

- [x] **Create widgets.yaml with Docker and system widgets** `[agent:docker]`
  ...
  - **Completed**: Created `stacks/homepage/config/homepage/widgets.yaml` with resources widget (CPU, memory, disk) and search widget. Docker widget configured in services.yaml.
